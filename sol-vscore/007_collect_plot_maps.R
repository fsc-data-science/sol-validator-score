...
add_coordinate_to_validator <- function(validator_lvl_data, voter_coordinate, select_epoch = NULL){
  ...
  vcll <- merge(validator_lvl_data, voter_coordinate, all.x = TRUE, all.y = FALSE, by = c("epoch","voter_pubkey"))
  ...
  vcll$latitude[is.na(vcll$latitude)] <- 0
  vcll$longitude[is.na(vcll$longitude)] <- 0
...
add_country_to_validator <- function(validator_lvl_data, voter_country, world_map, select_epoch = NULL){
 ...
 if(!is.null(select_epoch)){
   vctry <- vctry %>% filter(epoch == select_epoch)
 }
...
plot_country <- function(country_geo_data, color_col, hover_text_col){
  ...
  colorpal <- colorNumeric(palette = "viridis", domain = country_geo_data[[color_col]])
